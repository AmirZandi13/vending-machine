...
def test_get_machine_by_name_when_machine_exists():
    ...
    machine = MachineService().get_machine_by_name(machine_name, machines=machines)
...
def test_machine_is_available_when_machine_is_in_idle_state():
    machine_name = "machine-1"
    machine_is_available = MachineService().machine_is_available(machine_name, machines=machines)
...
def test_machine_is_available_when_machine_is_not_in_idle_state():
    ...
    machine_is_available = MachineService().machine_is_available(machine_name, machines=machines_data)
...
def test_change_state_of_machine():
    ...
    machines_data_after_change_the_state = MachineService().change_state(machines_data[0]["id"], Machine().RUNNING,
                                                                            machines_data)
